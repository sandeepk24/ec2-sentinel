...
export function DiagnosisPanel({ report }: { report: Report }) {
  ...
  const breakdownSummary = [
    topCpu ? `CPU: ${topCpu.name} ${topCpu.cpu_percent}%` : null,
    topMem ? `Mem: ${topMem.name} ${topMem.memory_mb} MB` : null,
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    <div className="space-y-4">
      <motion.div
        ...
        className={`relative overflow-hidden rounded-3xl border bg-gradient-to-br p-5 sm:p-6 backdrop-blur-xl ${healthStyle}`}
      >
        ...
        <div className="relative">
          ...
          <h3 className="font-display text-xl font-extrabold tracking-tight text-slate-900 sm:text-2xl dark:text-white">
            {d.headline}
          </h3>
          <p className="mt-2 max-w-3xl text-base font-medium leading-relaxed text-slate-700 dark:text-slate-200">
            {d.summary}
          </p>
        </div>
      </motion.div>
